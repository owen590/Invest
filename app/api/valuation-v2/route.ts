...
export async function GET(){
 const results=await Promise.all(SYMBOLS.map(async symbol=>{
  ...
  const valuationState=valuationScore==null?'暂无数据':valuationScore>=80?'深度低估':valuationScore>=65?'低估':valuationScore>=45?'合理':valuationScore>=30?'偏高':'高估'
  ...
  return{symbol,ok:metrics.some(m=>m.current!==null),valuationScore,valuationTemperature:temperature,valuationQuality:quality,valuationState,metrics,price,high52:high,low52:low,pricePosition,priceAttractiveness,priceState,methodology:'V1.8：估值温度由 P/E 30%、P/FCF 20%、EV/EBITDA 15%、FCF Yield 15%、P/S 10%、Earnings Yield 10% 的5年历史分位组成；估值质量衡量指标一致性；Long Call 估值再加入价格吸引力修正。'}
 }))
